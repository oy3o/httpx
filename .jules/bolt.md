...
## 2026-04-23 - [Reuse Global Slice for Static Headers if cap == len]
**Learning:** It is safe to reuse a global or closure-captured slice for static headers (e.g. `h["Vary"] = sharedSlice`) as long as the slice's capacity equals its length (e.g. created via a slice literal `[]string{"Origin"}`). Because `cap == len`, any downstream `.Add()` call which uses `append` will be forced to allocate a new backing array rather than mutating the shared array. However, we found that this still causes allocations when setting the same headers dynamically during request time since we were allocating new slices like `[]string{value}` to bypass `Set()` string formatting overhead.
**Action:** We used slice pre-allocation inside the outer closure of middlewares (like CORS and SecurityHeaders) for static headers. For instance, in CORS we changed dynamic `[]string{val}` slice creation to pre-calculated slice creation in the outer closure. For static variables like `allowOriginStarSlice := []string{"*"}` that's pre-calculated, map assignments like `h["Access-Control-Allow-Origin"] = allowOriginStarSlice` are 100% allocation-free and avoid `textproto.CanonicalMIMEHeaderKey`.
